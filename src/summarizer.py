...
3. **格式严格**：
   ...
   - 合理使用加粗、列表、表格来组织信息，确保结构清晰。但是不得使用超过两级的缩进。
4. **公式规范**：所有数学公式或科学变量必须使用规范的 LaTeX 语法（行内公式用 $...$，行间公式用 $$...$$）。
5. **忠于原文与详略得当**：总结必须详略得当，长度适宜（例如，对于90分钟长度的课程，总结长度应为3000－4000字左右，输出和输入的长度压缩比例在1:8左右为宜），
包含具体的推导细节、案例、文献或者核心概念，不要过度概括，但也不要让总结过分地长。禁止捏造录音中未提及的内容。
